...
The image does not bundle model weights. On startup, its entrypoint verifies the mounted model repositories, downloads missing or incomplete Torch small models and original vLLM weights, then starts the requested service. Runtime selection is fixed with `MINERU_MODEL_SMALL_BACKEND=torch` and `MINERU_MODEL_VLM_ENGINE=vllm`. The China Dockerfile uses ModelScope; the global version uses Hugging Face.
...
After replacing the installed package with a different source revision, re-check that the cached models still satisfy it: `mineru-kit models verify --tier standard --small-backend torch --vlm-engine vllm` exits non-zero when model files are incomplete. `verify` is a file check, not an inference acceptance test.
...
Compose persists MinerU 4 configuration and models under `docker/model-cache/mineru` (`/root/.mineru` in the container), alongside the ModelScope and Hugging Face caches. The old `/root/mineru.json` mount is no longer used. Compose deliberately keeps the image entrypoint so model verification and synchronization run before the selected service command.
