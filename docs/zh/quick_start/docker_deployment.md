...
镜像本身不包含模型权重。容器启动时，入口脚本先校验挂载目录中的模型仓库；Torch 小模型或 vLLM 原始权重缺失、不完整时才执行下载，然后启动指定服务。运行时固定 `MINERU_MODEL_SMALL_BACKEND=torch`、`MINERU_MODEL_VLM_ENGINE=vllm`。国内 Dockerfile 使用 ModelScope，国际版本使用 Hugging Face。
...
替换安装包为其他源码版本后，重新确认缓存中的模型是否仍满足要求：`mineru-kit models verify --tier standard --small-backend torch --vlm-engine vllm` 在模型文件不完整时以非零退出码失败。`verify` 是文件检查，不是推理验收。
...
Compose 将 MinerU 4 配置和模型持久化到 `docker/model-cache/mineru`（容器内 `/root/.mineru`），同时保留 ModelScope、Hugging Face 缓存。旧的 `/root/mineru.json` 挂载已不再使用。Compose 会保留镜像入口，因此模型校验与同步会在所选服务命令之前执行。
